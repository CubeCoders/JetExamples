...
p.add_argument('--seconds',type=float,default=181)
a=p.parse_args();a.output.parent.mkdir(parents=True,exist_ok=True)
if a.renderer is None:
 a.renderer=Path(__file__).resolve().parents[1]/(('build-quality/matter_quality' if a.quality else 'build-preview/matter_video')+('.exe' if os.name=='nt' else ''))
