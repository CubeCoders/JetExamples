...
![Rotating cube: ESP32-style screenshot, performance overlay hidden](../docs/screenshots/esp32-template-cube.png)
...
Half-width output can be disabled, at twice the framebuffer memory cost. The S3
worker supports row-disjoint depth rendering too. Parallel water must sample the
previous field buffer, which the runtime supplies.
...
a frame loop with invalid buffers. Serial statistics are printed every 60 fields.
