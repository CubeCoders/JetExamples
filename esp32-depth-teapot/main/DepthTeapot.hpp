...
#include "Scene.hpp"
#include "Display.hpp"
#include "../../esp32-lighting-teapot/main/TeapotMesh.hpp"
#include "Labels.hpp"
#include <cmath>
#include <cstdio>

namespace DepthTeapot {
...
inline void selectMode(int mode) {
    ...
    activeMode = mode;
    targetScene->getRenderer()->setDepthTestingEnabled(mode == 1);
    Labels::select(mode);
    std::printf("Depth mode: %s\n",mode ? "DEPTH TESTING" : "PAINTER SORTING");
}
...
inline void update(float seconds) {
    time = std::fmod(time + seconds, 3584.0f);
    selectMode(int(time / 7.0f) % 2);
    // Each seven-second phase follows exactly the same seamless motion loop.
    // Only visibility handling changes; geometry, material and light stay fixed.
