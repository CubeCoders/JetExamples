...
# Match World.hpp: deeper shadows and brighter highlights, pivot at 80/255.
def contrast_channel(c):return max(0,min(255,(c*5-80+2)//4))
...
for k in range(4):
 ...
 s+=array('facade'+str(k),im.transpose(Image.Transpose.FLIP_TOP_BOTTOM));save_art(im,r/f'assets/facade-{k}.png')
...
for k in range(4):
 ...
 s+=array('shop'+str(k),im.transpose(Image.Transpose.FLIP_TOP_BOTTOM));save_art(im,r/f'assets/shop-{k}.png')
...
for i,(a,b,c) in enumerate(signs):
 ...
 d.text((7,0),a,font=font(28),fill=c);d.text((8,30),b,font=font(12),fill=(215,231,240));im=im.resize((128,64),Image.Resampling.NEAREST);s+=array('sign'+str(i),im.transpose(Image.Transpose.FLIP_TOP_BOTTOM));save_art(im,r/f'assets/sign-{i}.png')
...
d.line((0,40,127,40),fill=(165,91,198),width=4);s+=array('environment',im);save_art(im,r/'assets/environment.png')
...
d.ellipse((22,12,42,32),outline=(103,250,255),width=2);d.polygon([(21,38),(43,38),(49,75),(15,75)],outline=(92,230,238));s+=array('hologram',im.transpose(Image.Transpose.FLIP_TOP_BOTTOM));save_art(im,r/'assets/hologram.png')
...
im=im.resize((256,128),Image.Resampling.NEAREST);s+=array('dashboard',im.transpose(Image.Transpose.FLIP_TOP_BOTTOM));save_art(im,r/'assets/dashboard.png')
